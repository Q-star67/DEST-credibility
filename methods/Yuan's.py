...
def dict_to_vector(d, frame_of_discernment):
    subsets = list(frame_of_discernment.keys())
    return np.array([d.get(subset, 0) for subset in subsets])
...
Sim = 1 - jousselme_distance_matrix
Sup = np.zeros(n)
for i in range(n):
    for j in range(n):
        if i != j:
...
# Calculate Crd for each evidence
